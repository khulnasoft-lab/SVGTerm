...
# does not seem worth it: it adds a dependency and slows down the invocation of svgterm
# by 150ms ('time svgterm --help' execution time goes from 200ms to 350ms)
...
def validate_geometry(screen_geometry):
    ...
    if columns <= 0 or rows <= 0:
        raise ValueError(
            'Invalid value for screen-geometry option: "{}"'.format(screen_geometry)
        )
    return columns, rows
