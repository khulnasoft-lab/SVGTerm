...
def _read_v1_records(data):
    v1_header_attributes = {"version", "width", "height", "stdout"}
    try:
    ...
    if missing_attributes:
        raise AsciiCastError(
            "Missing attributes in asciicast v1 file: {}".format(missing_attributes)
        )
    ...
    if not isinstance(json_dict["stdout"], Iterable):
        raise AsciiCastError(
            "Invalid type for stdout attribute (expected Iterable): {}".format(
                json_dict["stdout"]
            )
        )
    ...
    for event in json_dict["stdout"]:
        ...
        if not isinstance(time_elapsed, (int, float)) or not isinstance(
            event_data, str
        ):
            raise AsciiCastError(
                'Invalid type for event: got object "{}" but expected '
        ...
        if cls.is_color(fg):
            if cls.is_color(bg):
                ...
                if len(colors) >= 8 and all([cls.is_color(c) for c in colors[:8]]):
                    ...
                    return self
                raise AsciiCastError(
                    "Invalid palette: the first 8 or 16 colors must be valid"
                )
...
_AsciiCastV2Header = namedtuple(
    "AsciiCastV2Header", ["version", "width", "height", "theme", "idle_time_limit"]
)
...
class AsciiCastV2Header(AsciiCastV2Record, _AsciiCastV2Header):
    ...
    def __new__(cls, version, width, height, theme, idle_time_limit=None):
        self = super(AsciiCastV2Header, cls).__new__(
            cls, version, width, height, theme, idle_time_limit
        )
        for attr_name in cls._fields:
            attr = self.__getattribute__(attr_name)
            if not isinstance(attr, cls.types[attr_name]):
                raise AsciiCastError(
                    "Invalid type for attribute {}: {} (expected one of {})".format(
                        attr_name, type(attr), cls.types[attr_name]
                    )
                )
        if version != 2:
    ...
    def from_json_line(cls, line):
        ...
        filtered_attributes = {
            attr: attributes.get(attr) for attr in AsciiCastV2Header._fields
        }
        if filtered_attributes["theme"] is not None:
            filtered_attributes["theme"] = AsciiCastV2Theme(
                **filtered_attributes["theme"]
            )
...
_AsciiCastV2Event = namedtuple(
    "AsciiCastV2Event", ["time", "event_type", "event_data", "duration"]
)
