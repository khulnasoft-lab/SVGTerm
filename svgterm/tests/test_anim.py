...
from svgterm import term


TEMPLATE = pkgutil.get_data("svgterm", "/data/templates/gjm8.svg")
...
class TestAnim(unittest.TestCase):
    ...
    def test__render_line_bg_colors_xml(self):
        ...
        rectangles = anim._render_line_bg_colors(
            screen_line=screen_line, height=0, cell_height=1, cell_width=cell_width
        )
    ...
    def test__render_characters(self):
        screen_line = {
            ...
            60: anim.CharacterCell(
                "L", "black", "black", underscore=True, strikethrough=True
            ),
        }
        ...
        with self.subTest(case="Content"):
            cell_width = 8
            texts = {
                t.text: t for t in anim._render_characters(screen_line, cell_width)
            }
    ...
    def test__render_timed_frame(self):
        ...
        all_definitions = {}
        for frame in frames:
            group, new_defs = anim._render_timed_frame(
                offset=0,
                buffer=frame.buffer,
                cell_width=8,
                cell_height=17,
    ...
    def test_render_animation(self):
        ...
        _, filename = tempfile.mkstemp(prefix="svgterm_", suffix=".svg")
    ...
    def test__render_still_frames(self):
        ...
        def extract_text_content(frame):
            svg_screen_elem = frame.find(
                './/{{{namespace}}}svg[@id="screen"]'.format(namespace=anim.SVG_NS)
            )
    ...
    def test__embed_css(self):
        test_cases = [
            {"animation_duration": None, "timings": {1: 100, 2: 200}},
    ...
    def test_validate_svg(self):
        failure_test_cases = ["", "<svg>", "</svg>", "</svg></a>", None]
