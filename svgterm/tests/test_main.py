...
class TestMain(unittest.TestCase):
    ...
    def test_main(self):
        ...
        with self.subTest(
            case="record and render on the fly (window_frame_js template)"
        ):
        ...
        for template in svgterm.config.default_templates():
            with self.subTest(
                case="record and render on the fly ({} template)".format(template)
            ):
    ...
    def test_integral_duration(self):
        ...
        for case in test_cases:
            with self.subTest(case=case):
                self.assertEqual(svgterm.main.integral_duration_validation(case), 100)
