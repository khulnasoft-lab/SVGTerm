...
import svgterm.anim as anim
from svgterm import term
from svgterm.asciicast import AsciiCastV2Event, AsciiCastV2Header, AsciiCastV2Theme
...
class TestTerm(unittest.TestCase):
    ...
    def test_timed_frames_simple_events(self):
        records = [AsciiCastV2Header(version=2, width=80, height=24, theme=THEME)] + [
            AsciiCastV2Event(
                time=i, event_type="o", event_data="{}\r\n".format(i), duration=None
            )
            for i in range(0, 2)
        ]
        ...
        for expected_frame, frame in z:
            ...
            for row in frame.buffer:
                if row in expected_frame.buffer:
                    self.assertEqual(expected_frame.buffer[row], frame.buffer[row])
                else:
    ...
    def test_timed_frames_unprintable_chars(self):
        ...
        # Issue https://github.com/khulnasoft-lab/svgterm/issues/89

        # test_text = "e🕵️‍a"
        test_text = (
            b"e"
            + b"\xf0\x9f\x95\xb5"
            + b"\xef\xb8\x8f"  # sleuth emoji
            + b"\xe2\x80\x8d"  # variation selector 16
            + b"a"  # zero width joiner
        ).decode("utf-8")  # character that should be preserved
        ...
        frame = next(events)
        characters = "".join(frame.buffer[0][col].text for col in frame.buffer[0])
