...
_CharacterCell.__new__.__defaults__ = (
    "foreground",
    "background",
    False,
    False,
...
def render_animation(
    frames, geometry, filename, template, cell_width=CELL_WIDTH, cell_height=CELL_HEIGHT
):
...
def _render_preparation(geometry, template, cell_width, cell_height):
    ...
    svg_screen_tag = root.find(
        './/{{{namespace}}}svg[@id="screen"]'.format(namespace=SVG_NS)
    )
...
def _render_still_frames(frames, root, cell_width, cell_height):
    for frame in frames:
        ...
        svg_screen_tag = frame_root.find(
            './/{{{namespace}}}svg[@id="screen"]'.format(namespace=SVG_NS)
        )
        if svg_screen_tag is None:
            raise ValueError('Missing tag: <svg id="screen" ...>...</svg>')
...
def _add_animation(root, timings, animation_duration):
    ...
    settings = root.find(
        ".//{{{}}}defs/{{{}}}template_settings".format(SVG_NS, SVGTERM_NS)
    )
    if settings is None:
    ...
    if animation is None:
        raise TemplateError(
            'Missing or invalid "animation" element in "template_settings"'
        )

    f = animators.get(animation.attrib["type"].lower())
    if f is None:
        raise TemplateError(
            "Attribute 'type' of element 'animation' must be one of {}".format(
                ", ".join(animators.keys())
            )
        )
...
def _render_timed_frame(offset, buffer, cell_height, cell_width, definitions):
    ...
    for row_number in buffer:
        if buffer[row_number]:
            ...
            tags, new_definitions = _render_line(
                offset,
                row_number,
                buffer[row_number],
                cell_height,
...
def _render_line_bg_colors(screen_line, height, cell_height, cell_width):
    ...
    :param cell_width: Width of a character cell in pixels
    """
    non_default_bg_cells = [
        (column, cell)
        for (column, cell) in sorted(screen_line.items())
...
def _render_characters(screen_line, cell_width):
    ...
    key = ConsecutiveWithSameAttributes(
        ["color", "bold", "italics", "underscore", "strikethrough"]
    )
    text_tags = [
        _make_text_tag(
...
def resize_template(template, geometry, cell_width, cell_height):
    ...
    def scale(element, template_columns, template_rows, columns, rows):
        ...
        element.attrib["viewBox"] = " ".join(
            map(str, (vb_min_x, vb_min_y, vb_width, vb_height))
        )
        ...
        for attribute, delta in scalable_attributes.items():
            if attribute in element.attrib:
                try:
                    element.attrib[attribute] = str(
                        int(element.attrib[attribute]) + delta
                    )
                except ValueError:
                    raise TemplateError(
                        '"{}" attribute of {} must be in user units'.format(
                            attribute, element
                        )
                    )
        return element
    ...
    svg_geometry = settings.find(
        "{{{}}}screen_geometry[@columns][@rows]".format(SVGTERM_NS)
    )
    if svg_geometry is None:
        raise TemplateError('Missing "screen_geometry" element in "template_settings"')

    attributes_err_msg = (
        'Missing or invalid "columns" or "rows" attribute '
    ...
    svg_geometry.attrib["columns"], svg_geometry.attrib["rows"] = (
        str(columns),
        str(rows),
    )
...
def _embed_css(root, timings=None, animation_duration=None):
    try:
        style = root.find(
            './/{{{ns}}}defs/{{{ns}}}style[@id="generated-style"]'.format(ns=SVG_NS)
        )
    except etree.Error as exc:
        raise TemplateError("Invalid template") from exc
    ...
    if style is None:
        raise TemplateError(
            'Missing <style id="generated-style" ...> element ' 'in "defs"'
        )
    ...
    if animation_duration is None or timings is None:
        ...
        for time, offset in sorted(timings.items()):
            transforms.append(
                transform_format.format(
                    time=100.0 * time / animation_duration, offset=offset
                )
            )
            last_offset = offset

        if last_offset is not None:
            transforms.append(transform_format.format(time=100, offset=last_offset))
        ...
        """.format(duration=animation_duration, transforms=os.linesep.join(transforms))
...
def _embed_waapi(root, timings=None, animation_duration=None):
    ...
    if animation_duration and timings:
        ...
        script_element = root.find(
            './/{{{ns}}}script[@id="generated-js"]'.format(ns=SVG_NS)
        )
        if script_element is None:
            raise TemplateError('Missing <script id="generated-js" ...> element')

        transform_no_offset = (
            "{{transform: 'translate3D(0, {y_pos}px, 0)', easing: 'steps(1, end)'}}"
        )
        ...
        for time, y_pos in sorted(timings.items()):
            if last_pos is None:
                ...
                transforms.append(
                    transform_with_offset.format(
                        offset=time / animation_duration, y_pos=y_pos
                    )
                )
            last_pos = y_pos
...
def validate_svg(svg_file):
    ...
    try:
        ...
        for bad in tree.xpath(
            "/svg:svg/svg:defs/svgterm:template_settings", namespaces=NAMESPACES
        ):
